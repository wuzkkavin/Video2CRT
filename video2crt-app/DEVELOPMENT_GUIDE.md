...
| 高品質模型安裝 | NSIS + Python staging | `src-tauri/installer-hooks.nsh`、`scripts/install-large-model.py`、`src-tauri/src/model_manager.rs` | 安裝時確認、下載、manifest 完整性檢查；運行介面不提供模型選擇 |
...
高品質模型的選擇只在 NSIS 安裝程序中出現。使用者選擇下載時，安裝器呼叫 bundled Python 下載約 3.2 GB 並寫入 manifest；選擇略過則保留標準模型。主程式啟動與開始轉檔時不會觸發模型提示。
...
目前主功能、字幕模式、翻譯模式、Credential Manager 儲存、標題輸出資料夾、下載回退、self-contained NSIS 發布包與安裝時高品質模型選擇皆已實作。已知仍需完成的工作是：用乾淨 Windows 使用者帳戶驗證安裝流程、實際下載高品質模型，以及在最新程式碼上完成一次 GUI 到 `final.mp4` 的完整視覺 E2E。
