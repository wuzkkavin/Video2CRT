...
- Evidence: `src/pages/UrlPage.tsx`、`OptionsPage.tsx`、`ProgressPage.tsx`、`DonePage.tsx`、`ModelInstallDialog.tsx`

## 導覽與畫面

1. **URL Page**：輸入單一公開 YouTube URL，按「下一步」。空值或格式錯誤不得進入選項頁。
2. **Options Page**：裁切 `W:H:X:Y`、ASR 語言、字幕輸出、翻譯方式、模型與輸出資料夾；「開始轉檔」啟動後鎖定。
3. **Model Install Dialog**：字幕需要且高品質模型缺少時顯示估計容量、網路需求、標準模型、安裝與稍後選項；無字幕模式不得提示。
...
所有頁面需有正常、載入、空值、錯誤、取消與完成狀態。工作期間禁止重複提交；模型下載中的取消只停止本次下載，不可啟用半成品。錯誤訊息需指出使用者可處理的下一步，不能要求使用者安裝系統依賴。

## 內容與視覺契約

- 繁中介面使用明確動詞：下一步、開始轉檔、使用標準模型、安裝高品質模型、稍後。
